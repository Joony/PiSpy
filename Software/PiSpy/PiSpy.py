            ...
            if timedImageMode.get() == 1 or timedVideoMode.get() == 1: #timed mode selected
                ...
                if video.get() == 1: #print selected settings
                    ...
                    while len(self.times) > 0: #run program with selected settings
                        ...
                        time_thread = s.enter(20.25, 1, times.checkVidTime(str(ID+'video'+str(start_length-len(self.times)).zfill(5)+'_'), self.captureLength, self.resolution, self.framerate))
                    ...
                    if ROFFTime != '':
                        print('Red lights off at ' + ROFFTime)
                    start_length = len(self.image_times)
                    while len(self.image_times) > 0: #run program with selected settings using scheduler
                        ...
                        image_time_thread = s.enter(20.25, 1, times.checkImageTime(str(ID+'image'+str(start_length-len(self.image_times)).zfill(5)+'_'), self.resolution))
